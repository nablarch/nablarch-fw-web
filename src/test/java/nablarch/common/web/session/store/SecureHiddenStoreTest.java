package com.nablarch.example.app.web.session.store;

import com.nablarch.example.app.web.session.MockHttpServletRequest;
...
/**
 * {@link SecureHiddenStore}のテスト。
 *
 * {@link HiddenStore}との互換性を確認するため、{@link HiddenStore}のテストコードをそのまま流用している。
 * {@link SecureHiddenStore}独自実装部分のテストについては、本クラスの後半に記載している。
 *
 * @author Tsuyoshi Kawasaki
 */
public class SecureHiddenStoreTest {
    ...
    private SecureHiddenStore store;

    @Before
    public void setUp() {
        store = new SecureHiddenStore();
    ...
    public void testEncryption() throws UnsupportedEncodingException {
        SecureHiddenStore store = new SecureHiddenStore();
        store.setStateEncoder(new JavaSerializeStateEncoder());
    ...
    public void testInvalidKeyData() {
        ...
        ServletExecutionContext outCtxt = createExeCtxt();
        {
            ...
            // 別の鍵を持つSecureHiddenStoreで暗号化
            SecureHiddenStore another = new SecureHiddenStore();
